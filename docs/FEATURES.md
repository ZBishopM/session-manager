...
- [x] **v9 (live, 85ef1b6)** — **Bug fix — the field never actually worked.** Clicking "Guardar" always silently failed: PocketBase rejects a plain `email` update on an auth-collection record from a non-superuser no matter what's sent (confirmed 2026-09-04), and the old code had no error handling, so nothing happened and nothing was ever saved. Changing your email now goes through PocketBase's real flow: "Guardar" calls `requestEmailChange()` and shows "📩 revisa tu correo", you click the link in the email (points at a new `/confirm-email` page, not PocketBase's admin-only default), type your 4-digit passcode there to finish it. A persistent "✓ Confirmado: x@x.com" line shows your actual on-file email on every visit, independent of the input.
...
- [x] **v9 (live, 85ef1b6)** — **Fixes.** Your active code now survives a page reload (previously it only existed in memory right after generating — reloading /profile left you with no way to see your current code at all without regenerating it). Regenerating an existing code is now soft-locked behind an explicit "¿Seguro?" confirmation step, since it invalidates whatever's currently linked in Piles. The section also shows "Conectado como {nickname}" so it's unambiguous which account a code belongs to. A copy button was also added for the code itself.
