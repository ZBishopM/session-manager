...
/**
 * Lógica del emparejamiento semanal, en un módulo aparte a propósito.
 *
 ...
 * `function runWeeklyMatchmaker()` en el nivel superior de
 * weekly_matchmaker.pb.js y el cron reventaba cada domingo a las 18:00 con
 * `ReferenceError: runWeeklyMatchmaker is not defined`, tragado por el
 * try/catch del propio cron: el correo semanal no se envió nunca.
 *
 * Mismo caso y misma solución que voting_close.js. El ámbito superior de un
 * módulo cargado con require() SÍ sobrevive (es un closure normal de
 * CommonJS, como demuestra _core.js); lo que no sobrevive es el del fichero
 * de hooks.
 *
...
module.exports = {
  ...
  /** Agrupa disponibilidades, crea propuestas + invitaciones y notifica. */
  runWeeklyMatchmaker() {
    ...
    for (const group of groups) {
      ...
      // PocketBase guarda las fechas como "2006-01-02 15:04:05.000Z" — con
      // espacio, no con "T" — así que un ISO crudo en el filtro no casa
      // NUNCA: este guardia contra duplicados era código muerto y cada
      // ejecución repetía propuesta e invitaciones. No se notó porque el
      // cron llevaba meses sin llegar a ejecutarse (ReferenceError).
      const existing = $app.findRecordsByFilter(
        "match_proposals",
        `host = "${group.host}" && weekday = "${group.weekday}" && proposed_date = "${proposedDateIso.replace("T", " ")}"`,
        "",
        1,
        0,
      );
